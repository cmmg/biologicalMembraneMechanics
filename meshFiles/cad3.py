...
def circle(radius=1, center=None, angle=None):
    ...
    if angle is None:
        ...
        Cw[:,:] = [[ 1*9, 0, 0, 9], [ 1, 2, 0, 1], [ -1*1.0/3, 2*1.0/3, 0, 1.0/3],
                   [-1*1.0/3, -2*1.0/3, 0, 1.0/3], [1, -2, 0, 1], [1*9 ,0 ,0 ,9]]
        Cw[:,:2] *= radius
        
    else:
        raise Exception('For angle<360 new C^1 knots not yet implemented', 'For angle<360 new C^1 knots not yet implemented')
...
def revolve(nrb, point, axis=2, angle=None):
    ...
    matX = scipy.io.loadmat('circle.mat')
